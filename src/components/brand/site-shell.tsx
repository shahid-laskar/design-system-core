...
const navItems = ["The collection", "Our approach", "Journal"];
    ...
    <div className="min-h-screen overflow-x-clip bg-background text-foreground">
      ...
      <header className="border-b border-border bg-background">
        <PageContainer className="grid h-18 grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-2 lg:flex lg:justify-between">
              ...
              <nav className="flex flex-col px-6 py-5" aria-label="Mobile navigation">
                {navItems.map((item) => (
                  <SheetClose asChild key={item}>
                    <a href="/" className="border-b border-border py-5 font-display text-2xl transition-colors hover:text-primary">
                      {item}
                    </a>
                  </SheetClose>
                ))}
          ...
          <Link to="/" className="min-w-0 justify-self-center lg:shrink-0" aria-label="Sukoon House home">
            <BrandMark />
          </Link>

          <nav className="hidden items-center gap-7 lg:flex" aria-label="Primary navigation">
            {navItems.map((item) => <a key={item} href="/" className="nav-link">{item}</a>)}
          </nav>
      ...
      <footer className="border-t border-border bg-primary text-primary-foreground">
        ...
        <div className="border-t border-primary-foreground/15">
          <PageContainer className="flex flex-col gap-3 py-5 text-xs text-primary-foreground/60 sm:flex-row sm:items-center sm:justify-between">
            <p>© 2026 Sukoon House.</p><p>Privacy · Terms · Accessibility</p>
          </PageContainer>
        </div>
      </footer>
    </div>
